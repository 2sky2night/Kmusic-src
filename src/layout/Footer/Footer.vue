...
async function musicSetAfter() {
  ...
  try {
    ...
    if (resCheck.success) {
      ...
      // 将歌曲的基本信息保存在仓库中
      musicStore.addHistory({
        ...resSongInfor.songs[0],
        privilege: resSongInfor.privileges[0],
      });
...
onMounted(() => {
  if (musicStore.playingSong.id) {
    // 根据歌曲id获取当前播放的歌曲
    const index = musicStore.songList.findIndex(
      (ele) => ele.id === musicStore.playingSong.id
    );
