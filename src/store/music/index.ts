...
const useMusicStore = defineStore("music", {
  ...
  actions: {
    ...
    setPlayingSong(data: any) {
      ...
      if (window.navigator.mediaSession) {
        navigator.mediaSession.metadata = new MediaMetadata({
          title: data.name,
          artist: data.artists.length ? data.artists[0].name : "网络歌手",
      ...
      this.playingSong.isVip
        ? message("您正在播放vip歌曲~😎", "info", () => {}, icon)
        : message("正在播放 " + data.name, "info", () => {}, icon);
    },
    /**
     * 重置当前播放的歌曲
     */
    ...
    preSong() {
      // 获取到当前播放的歌曲
      let index = this.songList.findIndex(
        (ele) => ele.id === this.playingSong.id
      );
      if (index <= 0) {
